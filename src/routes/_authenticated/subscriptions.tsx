        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-left text-xs uppercase text-muted-foreground">
              <tr><th className="p-4">Socio</th><th className="p-4">Email</th><th className="p-4">Periodo</th><th className="p-4">Stato</th></tr>
            </thead>
            <tbody>
              ...
              {data?.map((s: any) => (
                <tr key={s.id} className="border-t border-border/40">
                  <td className="p-4 font-medium">{s.res_partner?.display_name ?? `${s.res_partner?.first_name ?? ""} ${s.res_partner?.last_name ?? ""}`.trim()}</td>
                  ...
                  <td className="p-4"><Badge className="rounded-full" variant={s.status === "active" ? "default" : "secondary"}>{s.status}</Badge></td>
                </tr>
              ))}
            </tbody>
          </table>
